...
// Lazy-load configuration variables (default: lazy load disabled)
const ssqLazyLoadEnabled = true
const ssqLazyLoadMode = 'scroll' // options: 'event' or 'scroll'
const ssqLazyLoadOffset = 100  // used only in 'scroll' mode (in pixels)


// const ssqLazyLoadEnabled = window.ssqLazyLoadEnabled || false;  // set true to enable lazy load non‑priority images
// const ssqLazyLoadMode = window.ssqLazyLoadMode || 'event';        // options: 'event' or 'scroll'
// const ssqLazyLoadOffset = window.ssqLazyLoadOffset || null;         // used only in 'scroll' mode (in pixels)
...
function preloadPriorityAndNonPriorityImages(){
    ...
    // NON PRIORITY IMAGE ARRAY SUBTRACT
    const subtractTwoArrays = (arr1, arr2) => arr1.filter( el => !arr2.includes(el) );
    ...
    // Handle non-priority images based on lazy-load settings:
    if ( ssqLazyLoadEnabled ) {
        if ( ssqLazyLoadMode === 'event' ) {
            console.log('Lazy load enabled: Waiting for custom event trigger for non-priority images.');
            document.addEventListener('loadNonPriority', loadNonPriorityImages);
        } else if ( ssqLazyLoadMode === 'scroll' && ssqLazyLoadOffset !== null ) {
            console.log('Lazy load enabled: Waiting for scroll offset of', ssqLazyLoadOffset, 'px to load non-priority images.');
            window.addEventListener("scroll", function onScroll() {
                if ( window.pageYOffset >= Number(ssqLazyLoadOffset) ) {
                    loadNonPriorityImages();
                    window.removeEventListener("scroll", onScroll);
                }
            });
        } else {
            // Fallback: invalid config, load non-priority immediately.
            loadNonPriorityImages();
        }
    } else {
        // Default behavior: load non-priority images immediately.
        loadNonPriorityImages();
    }
...
* loadNonPriorityImages
*/
function loadNonPriorityImages(){
    // NON PRIORITY IMAGE ARRAY PRELOAD
    for (var i = 0; i < ssqImg.nonPriorityImageArray.length; i++) {
        var shc = ssqImg.absoluteOrder[ ssqImg.nonPriorityImageArray[i] ][0];
        var str = ssqImg.absoluteOrder[ ssqImg.nonPriorityImageArray[i] ][1];
    ...
    console.log('Non-priority images are now being loaded.');
} // end of loadNonPriorityImages
