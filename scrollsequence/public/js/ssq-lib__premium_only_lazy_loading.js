// values for lazy loading
let lazyInitialScrollY = window.scrollY;
let lazyLoadPxValue = 100; // px
let lazyLoadTriggered = true;
console.log('LAZY LOAD: ',lazyLoadTriggered,'Lazy loading initialized with lazyInitialScrollY:', lazyInitialScrollY, 'and lazyLoadPxValue:', lazyLoadPxValue);
...
function preloadPriorityAndNonPriorityImages(){let t0 = performance.now();
  ...
  // FIX: Calculate nonPriorityImageCountRequired BEFORE slicing
  ...
  // Split non-priority images into "immediate" and "lazy"
  ssqImg.nonPriorityImmediate = ssqImg.nonPriorityImageArray.slice(0, ssqImg.nonPriorityImageCountRequired);
  ssqImg.nonPriorityLazy = ssqImg.nonPriorityImageArray.slice(ssqImg.nonPriorityImageCountRequired);

  // Load only the immediate non-priority images now
  for (var i=0; i< ssqImg.nonPriorityImmediate.length; i++) {
      var shc = ssqImg.absoluteOrder[ssqImg.nonPriorityImmediate[i]][0]
      var str = ssqImg.absoluteOrder[ssqImg.nonPriorityImmediate[i]][1] 
      var obr = ssqImg.absoluteOrder[ssqImg.nonPriorityImmediate[i]][2]  
  ...
  // NEW: Set up lazy loading for the rest
  if (ssqImg.nonPriorityLazy.length > 0) {
    lazyInitialScrollY = window.scrollY;
    lazyLoadTriggered = false;
    window.addEventListener('scroll', lazyLoadNonPriorityImages, {passive: true});
  }
...
// Add this new function near the other preload functions:
function lazyLoadNonPriorityImages() {
  if (lazyLoadTriggered) return;
  if (Math.abs(window.scrollY - lazyInitialScrollY) >= lazyLoadPxValue) {
    lazyLoadTriggered = true;
    window.removeEventListener('scroll', lazyLoadNonPriorityImages, {passive: true});
    // Load the remaining non-priority images
    for (var i=0; i< ssqImg.nonPriorityLazy.length; i++) {
      var shc = ssqImg.absoluteOrder[ssqImg.nonPriorityLazy[i]][0]
      var str = ssqImg.absoluteOrder[ssqImg.nonPriorityLazy[i]][1] 
      var obr = ssqImg.absoluteOrder[ssqImg.nonPriorityLazy[i]][2]  
      ssqImg.objArray[shc][str][obr].aaaaaaIsPriority=false;
      ssqImg.objArray[shc][str][obr].aaaImageInfo={sc:shc,page:str,image:obr}; 
      ssqImg.objArray[shc][str][obr].onload =preloadImagesCallbackFunction;
      ssqImg.objArray[shc][str][obr].src =ssqInput.sc[shc].page[str].imagesFull[obr];
    }
    PRELOADDEBUG && console.log(`Lazy loaded remaining non-priority images after ${lazyLoadPxValue}px scroll`);
  }
}
